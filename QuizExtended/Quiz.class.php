...
class Quiz {
	...
	public function __construct( $argv, &$parser ) {
		...
		$this->preview = ( array_key_exists( 'preview', $argv ) &&
			$argv['preview'] == 'true' );
		$this->grade = ( array_key_exists( 'grade', $argv ) ? intval($argv['grade']) : false );
	...
	function parseQuiz( $input ) {
		...
		// START OF EDITED STUFF
		
		// image stuff
		if ($this->mTotal == 7) {
			$img = '/extensions/QuizExtended/images/' . round(($this->mScore / $this->mTotal) * 100) . '.jpg';
		}
		else {
			$img = false;
		}
		...
		if( StubObject::isRealObject( $wgUser ) &&  $wgUser->isLoggedIn()) {
			$t = $this->mParser->getTitle();
			$ar = new Article($t);

			$db = wfGetDB( DB_MASTER );
			$res = $db->selectRow( //does the user have a score for this quiz?
				'qz_score',
				'sc_percent',
				array('sc_user_id' => $wgUser->getId(), 'sc_page_id' => $ar->getId())
			);
			if ($res) { // if they do use their previous score
				$percent = $res->sc_percent;
				$img = '/extensions/QuizExtended/images/' . $percent . '.jpg';
			} else if ($this->mBeingCorrected) { // if it has been submitted
				$db->insert(
					'qz_score',
					[
						'sc_user_id' => $wgUser->getId(),
						'sc_page_id' => $ar->getId(),
						'sc_percent' => round(($this->mScore / $this->mTotal) * 100),
						'sc_data' => $this->bitQuestions,
						'sc_timestamp' => $db->timestamp(wfTimestamp(TS_UNIX)),
						'sc_grade' => $this->grade ? $this->grade : null
					]); 
			}
		}
		...
		$h = $templateParser->processTemplate( // the templates have been edited
			'Quiz',
			[
				'quiz' => [
					'id' => $this->mQuizId,
		...
		if ( $lState != '' ) {
			...
			switch ( $lState ) {
				case 'right':
					...
					$this->bitQuestions = ($this->bitQuestions << 1) | 1;
					break;
				...
				case 'wrong':
					...
					$this->bitQuestions = ($this->bitQuestions << 1);
					break;
